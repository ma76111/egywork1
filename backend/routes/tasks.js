...
// Named routes BEFORE /:id to avoid Express matching them as IDs
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ message: 'تم نشر المهمة بنجاح', id: r.rows[0].id });
  } catch (e) { console.error(e); res.status(500).json({ message: 'error' }); }
});
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    const r = await db.query(
      ...
      [req.params.id]);
    if (!r.rows[0]) return res.status(404).json({ message: 'المهمة غير موجودة' });
    res.json(r.rows[0]);
  } catch (e) { console.error(e); res.status(500).json({ message: 'error' }); }
});
...
router.post('/:id/submit', authMiddleware, async (req, res) => {
  try {
    ...
    if (ex.rows[0]) return res.status(400).json({ message: 'قدمت هذه المهمة مسبقاً' });
...
router.get('/:id/submissions', authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      ...
      if (!tr.rows[0]) return res.status(403).json({ message: 'غير مسموح' });
    }
    const r = await db.query(
