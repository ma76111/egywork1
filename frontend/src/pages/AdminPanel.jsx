  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {[{v:'stats',l:'الإحصائيات'},{v:'withdrawals',l:`السحوبات (${withdrawals.length})`},{v:'users',l:'المستخدمون'}].map(t => (
      ...
      {tab === 'stats' && stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'سحوبات معلقة', value: `${stats.pendingWithdrawals?.count} (${stats.pendingWithdrawals?.total?.toFixed(0)} ج.م)`, emoji: '💸' },
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-bold ${u.role === 'admin' ? 'bg-red-100 text-red-600' : u.role === 'advertiser' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'}`}>{u.role === 'admin' ? 'أدمن' : u.role === 'advertiser' ? 'معلن' : 'عامل'}</span></td>
                  <td className="px-4 py-3 font-bold text-green-600">{u.balance?.toFixed(2)} ج.م</td>
